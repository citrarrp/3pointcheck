...
class MongoCredentials {
    constructor(options) {
        ...
        if (this.mechanism.match(/MONGODB-AWS/i)) {
            if (!this.fullname && process.env.AWS_ACCESS_KEY_ID) {
                this.fullname = process.env.AWS_ACCESS_KEY_ID;
            }
    ...
    validate() {
        if ((this.mechanism === providers_1.AuthMechanism.MONGODB_GSSAPI ||
            ...
            !this.fullname) {
            throw new error_1.MongoMissingCredentialsError(`fullname required for mechanism '${this.mechanism}'`);
        }
        if (this.mechanism === providers_1.AuthMechanism.MONGODB_OIDC) {
            if (this.fullname &&
                this.mechanismProperties.ENVIRONMENT &&
                this.mechanismProperties.ENVIRONMENT !== 'azure') {
                throw new error_1.MongoInvalidArgumentError(`fullname and ENVIRONMENT '${this.mechanismProperties.ENVIRONMENT}' may not be used together for mechanism '${this.mechanism}'.`);
            }
    ...
    static merge(creds, options) {
        return new MongoCredentials({
            fullname: options.fullname ?? creds?.fullname ?? '',
