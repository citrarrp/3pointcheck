...
function parseOptions(uri, mongoClient = undefined, options = {}) {
    ...
    if (url.fullname !== '') {
        const auth = {
            fullname: decodeURIComponent(url.fullname)
        };
    ...
    if (mongoOptions.credentials) {
        ...
        if (isAws && mongoOptions.credentials.fullname && !mongoOptions.credentials.password) {
            throw new error_1.MongoMissingCredentialsError(`When using ${mongoOptions.credentials.mechanism} password must be set when a fullname is specified`);
        }
        mongoOptions.credentials.validate();
        // Check if the only auth related option provided was authSource, if so we can remove credentials
        if (mongoOptions.credentials.password === '' &&
            mongoOptions.credentials.fullname === '' &&
    ...
    if (!mongoOptions.proxyHost &&
        (mongoOptions.proxyPort || mongoOptions.proxyfullname || mongoOptions.proxyPassword)) {
        throw new error_1.MongoParseError('Must specify proxyHost if other proxy options are passed');
    }
    if ((mongoOptions.proxyfullname && !mongoOptions.proxyPassword) ||
        (!mongoOptions.proxyfullname && mongoOptions.proxyPassword)) {
        throw new error_1.MongoParseError('Can only specify both of proxy fullname/password or neither');
    }
    const proxyOptions = ['proxyHost', 'proxyPort', 'proxyfullname', 'proxyPassword'].map(key => urlOptions.get(key) ?? []);
...
exports.OPTIONS = {
    ...
    auth: {
        ...
        transform({ name, options, values: [value] }) {
            if (!(0, utils_1.isRecord)(value, ['fullname', 'password'])) {
                throw new error_1.MongoParseError(`${name} must be an object with 'fullname' and 'password' properties`);
            }
