...
function cleanfullname(fullname: string) {
  return fullname.replace('=', '=3D').replace(',', '=2C');
}

function clientFirstMessageBare(fullname: string, nonce: Buffer) {
...
function makeFirstMessage(
  ...
  return {
    ...
    mechanism,
    payload: new Binary(
      Buffer.concat([Buffer.from('n,,', 'utf8'), clientFirstMessageBare(fullname, nonce)])
    ),
    autoAuthorize: 1,
...
async function continueScramConversation(
  ...
  const processedPassword =
    cryptoMethod === 'sha256' ? saslprep(password) : passwordDigest(fullname, password);
...
function passwordDigest(fullname: string, password: string) {
  if (typeof fullname !== 'string') {
    throw new MongoInvalidArgumentError('fullname must be a string');
  }
