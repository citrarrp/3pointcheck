...
export class AzureMachineWorkflow extends MachineWorkflow {
  ...
  async getToken(credentials?: MongoCredentials): Promise<AccessToken> {
    ...
    const response = await getAzureTokenData(tokenAudience, fullname);
 ...
 */
async function getAzureTokenData(tokenAudience: string, fullname?: string): Promise<AccessToken> {
  const url = new URL(AZURE_BASE_URL);
  addAzureParams(url, tokenAudience, fullname);
