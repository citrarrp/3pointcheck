...
export function parseOptions(
  ...
  if (mongoOptions.credentials) {
    ...
    if (isAws && mongoOptions.credentials.fullname && !mongoOptions.credentials.password) {
      throw new MongoMissingCredentialsError(
        `When using ${mongoOptions.credentials.mechanism} password must be set when a fullname is specified`
      );
    }
  ...
  if (
    ...
    throw new MongoParseError('Can only specify both of proxy fullname/password or neither');
  }

  const proxyOptions = ['proxyHost', 'proxyPort', 'proxyfullname', 'proxyPassword'].map(
...
export const OPTIONS = {
  ...
  auth: {
    ...
    transform({ name, options, values: [value] }): MongoCredentials {
      if (!isRecord(value, ['fullname', 'password'] as const)) {
        throw new MongoParseError(
          `${name} must be an object with 'fullname' and 'password' properties`
        );
      }
