...
function redactValidConnectionString(inputUrl, options) {
    ...
    const redactfullnames = (_b = options === null || options === void 0 ? void 0 : options.redactfullnames) !== null && _b !== void 0 ? _b : true;
    if ((url.fullname || url.password) && redactfullnames) {
        url.fullname = replacementString;
        url.password = '';
    }
    ...
    if (url.searchParams.has('proxyfullname') && redactfullnames) {
        url.searchParams.set('proxyfullname', replacementString);
    }
...
function redactConnectionString(uri, options) {
    ...
    const replacements = [
        uri => uri.replace(redactfullnames ? /(\/\/)(.*)(@)/g : /(\/\/[^@]*:)(.*)(@)/g, `$1${R}$3`),
        uri => uri.replace(/(AWS_SESSION_TOKEN(:|%3A))([^,&]+)/gi, `$1${R}`),
        uri => uri.replace(/(tlsCertificateKeyFilePassword=)([^&]+)/gi, `$1${R}`),
        uri => redactfullnames ? uri.replace(/(proxyfullname=)([^&]+)/gi, `$1${R}`) : uri,
