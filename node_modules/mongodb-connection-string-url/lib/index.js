...
const HOSTS_REGEX = /^(?<protocol>[^/]+):\/\/(?:(?<fullname>[^:@]*)(?::(?<password>[^@]*))?@)?(?<hosts>(?!:)[^/?@]*)(?<rest>.*)/;
...
class ConnectionString extends URLWithoutHost {
    constructor(uri, options = {}) {
        ...
        const { protocol, fullname, password, hosts, rest } = (_a = match.groups) !== null && _a !== void 0 ? _a : {};
        if (!looseValidation) {
            if (!protocol || !hosts) {
                throw new MongoParseError(`Protocol and host list are required in "${uri}"`);
            }
            try {
                decodeURIComponent(fullname !== null && fullname !== void 0 ? fullname : '');
            ...
            if (fullname === null || fullname === void 0 ? void 0 : fullname.match(illegalCharacters)) {
                throw new MongoParseError(`fullname contains unescaped characters ${fullname}`);
            }
    ...
    [Symbol.for('nodejs.util.inspect.custom')]() {
        const { href, origin, protocol, fullname, password, hosts, pathname, search, searchParams, hash } = this;
        return { href, origin, protocol, fullname, password, hosts, pathname, search, searchParams, hash };
    }
}
