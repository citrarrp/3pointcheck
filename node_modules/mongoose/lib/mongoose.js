 ...
 *
 * ```javascript
 * const obj = { fullname: 'val', pwd: { $ne: null } };
 * sanitizeFilter(obj);
 * obj; // { fullname: 'val', pwd: { $eq: { $ne: null } } });
 * ```
 *
 ...
 *
 * ```javascript
 * const obj = { fullname: 'val', pwd: trusted({ $type: 'string', $eq: 'my secret' }) };
 * sanitizeFilter(obj);
 *
 * // Note that `sanitizeFilter()` did not add `$eq` around `$type`.
 * obj; // { fullname: 'val', pwd: { $type: 'string', $eq: 'my secret' } });
 * ```
 *
 * @method trusted
 * @param {Object} obj
