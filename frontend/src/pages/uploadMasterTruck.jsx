...
const TrucksDataUpload = () => {
  ...
  //     const sodData = await fetchSODDiagram();
  //     console.log(sodData, "data isinya apa");
  //     const filtered = sodData.filter((item) => item.customerName === cust);

  //     const mapped = filtered.map((item, idx) => ({
  //       cycleNumber: (idx + 1).toString(),
  //       loadingTime: item.loadingTime ? new Date(item.loadingTime) : null,
  //       ETD: item.ETD ? new Date(item.ETD) : null,
  //       ETA: null,
  //       ETA_Cust: null,
  //     }));

  //     setCycleSteps(mapped);
  //   };
  ...
  const matchCustomer = (excelCust, availableCustomers) => {
    ...
    console.log(availableCustomers, normExcel, "itu siap");
    const found = availableCustomers.find((cust) =>
      normExcel.includes(normalizeCustomer(cust))
    );
    console.log(found, "dietmukan");
    return found || excelCust; // kalau ga ketemu, tetap pakai isi dari Excel
  };
  //   const buildCycleStepsFromSOD = (customerName) => {
  //     const cust = normalizeCustomer(customerName);
  //     if (!cust) return [];

  //     const filtered = dataSOD.filter(
  //       (x) => normalizeCustomer(x.customerName) === cust
  //     );

  //     return filtered.map((item, idx) => {
  //       const loadingRow = item?.find((p) =>
  //         normalizeCustomer(p.processName).includes("LOADING TRUCK")
  //       );
  //       const etdRow = item?.find((p) =>
  //         normalizeCustomer(p.processName).includes("TRUCK OUT")
  //       );

  //       return {
  //         cycleNumber: String(idx + 1),
  //         loadingTime: loadingRow ? toWIBISO(loadingRow) : null,
  //         ETD: etdRow ? toWIBISO(etdRow) : null,
  //         ETA: null,
  //         ETA_Cust: null,
  //       };
  //     });
  ...
  const toWIBISO = (val) => {
    ...
    const m = moment.tz(val, "Asia/Jakarta");
  ...
  const formatTimeValue = (value) => {
    ...
    try {
      ...
      // If can't parse as time, return original string
      return stringValue;
    } catch (error) {
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const mappedData = dataRow
        ...
        .map((row, rowIndex) => {
          ...
          headers.forEach((header, colIndex) => {
            ...
            console.log(
              `Row ${rowIndex}, Col ${colIndex}: Header="${normalizedHeader}", Value="${row[colIndex]}", MongoKey="${mongoKey}"`
            );
            ...
            if (normalizedHeader === "Customer") {
              ...
              console.log(
                matchCustomer(String(cellValue || ""), availableCustomers)
              );
      ...
      //       if (ow[rawKey] === "Customer") {
      //         const filtered = availableCustomers.filter((customer) =>
      //           row[rawKey].includes(customer)
      //         );
      //         row[rawKey] = filtered;
      //       }
      //       console.log(rawKey, "raw mnetah", row, row[rawKey]);
      //       const normalizedKey = cleanKey(rawKey);
      //       const mongoKey = fieldMapping[normalizedKey];

      //       if (!mongoKey) continue;
      //       console.log("mongokry", mongoKey, normalizedKey);
      //       let val = row[rawKey];

      //       if (normalizedKey === "Customer") {
      //         val = matchCustomer(row[rawKey], availableCustomers);
      //       }

      //       mappedRow[mongoKey] = val;
      //     }
      //     // const steps = buildCycleStepsFromSOD(mappedRow.Customer || "");
      //     // mappedRow.cycleSteps = steps;
      //     // mappedRow.lastSyncFromSOD = new Date().toISOString();

      //     return mappedRow;
      //   });

      console.log(mappedData, "ini conoth");
      setTruckData(mappedData);
    };

    reader.readAsBinaryString(file);
  };
  console.log("mapped", truckData);
  ...
  //   const fetchSODForRow = (idx) => {
  //     setTruckData((prev) => {
  //       const copy = [...prev];
  //       const r = copy[idx];
  //       const steps = buildCycleStepsFromSOD(r.customerName);
  //       r.cycleSteps = steps;
  //       r.lastSyncFromSOD = steps.length ? new Date().toISOString() : null;
  //       return copy;
  //     });
  //   };

  //   const setCycleField = (rowIdx, stepIdx, key, val) => {
  //     setTruckData((prev) => {
  //       const copy = [...prev];
  //       const st = copy[rowIdx].cycleSteps[stepIdx];
  ...
  const handleSubmit = async () => {
    ...
    // const totalRows = truckData.length;
    // for (let i = 0; i < truckData.length; i++) {
    //   const row = truckData[i];
    const payload = truckData.map((row) => ({
      ...row,
    }));
    try {
      console.log(payload, truckData, "contoh kirim");
      const res = await api.post("/trucks", payload);
      alert(`Selesai: ${res.data?.data || "OK"}`);
    } catch (err) {
      ...
      alert(`Upload selesai! data berhasil diunggah.`);
    }
  };
