...
const TimelineChart = ({
  ...
  const getTimeDelay = useMemo(() => {
    const fullDate = new Date(selectDate);
    fullDate.setMinutes(new Date(currentTime).getMinutes() + 15);
    const baseDate = fullDate ? new Date(fullDate) : new Date(currentTime);
  ...
  const allDepartures = useMemo(() => {
    return data
      .filter((item) => item.processName?.includes("TRUCK OUT"))
      .map((item) => ({
        ...item,
      ...
      .map((item, index) => {
        ...
        if (finishPrepTrack.status !== "-") {
          if (departureTrack?.status === "-") {
            percentage = 100;
            statusColor = "#fc5e03";
          } else if (departureTrack?.status.toLowerCase() === "Ontime") {
            count.ontime += 1;
            percentage = 100;
  ...
  const freezeRange = useMemo(() => {
    const redItems = filteredDeparture.filter(
