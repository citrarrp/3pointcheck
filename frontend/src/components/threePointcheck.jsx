...
export default function InputSmart() {
  ...
  const generateSummary = useCallback(
    (data, selectedFields, separator = "_") => {
      ...
      Data.filter(
        ...
        // comboFieldMap.set(comboKey, {
        //   qty: (comboFieldMap.get(comboKey).qty || 0) + totalQty,
        //   qtyKanban: (comboFieldMap.get(comboKey).qtyKanban || 0) + orderQty,
        // });
  ...
  function matchEntryWithMergedData(
    ...
    const kanbanMatch = kanban.length >= dataReal.kanbanlength;
      ...
      if (
        supplierParts.some(
          (part) =>
            material === part ||
            unique === part ||
            (part.includes(unique) && containsAll)
        )
      ) {
