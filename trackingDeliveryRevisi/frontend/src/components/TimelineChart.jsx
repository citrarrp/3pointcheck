...
// Plugin untuk selalu tampilkan tooltip di titik data
...
const TimelineChart = ({
  ...
  const getTimeRange = useCallback(() => {
    ...
    console.log(start, finalEnd, "waktu mulai end");
    return { start, end: finalEnd };
  }, [currentTime, selectDate, timeWindow]);

  const getTimeDelay = useMemo(() => {
    const fullDate = new Date(selectDate);
    fullDate.setMinutes(new Date(currentTime).getMinutes() + 2);
    const baseDate = fullDate ? new Date(fullDate) : new Date(currentTime);
    const startDelay = new Date(baseDate);
    console.log(startDelay, "waktu start");
    return startDelay;
  ...
  // useEffect(() => {
  //   console.log("TRACKING DATA CHECK:", trackingData);
  // }, [trackingData]);

  const allDepartures = useMemo(() => {
    // const filterData = trackingData
  ...
  const filteredDepartures = useMemo(() => {
    // const filtered = filteredDeparture.filter(
    //   (item) => item.waktuParsed >= start && item.waktuParsed <= end
    // );
    // return filtered;
    if (filteredDeparture.length == 0) return [];

    const departureInRange = filteredDeparture.filter((item) => {
    ...
    return [...departureInRange, ...departureDelay];
  }, [filteredDeparture, start, end, getTimeDelay]);

  // const departureDelay = useMemo(() => {
  //   const departure = filteredDeparture
  //     .filter((item) => item.statusColor === "#e60a29")
  //     .map((item) => ({
  //       ...item,
  //       waktuParsed: getTimeDelay,
  //       delay: item.waktuParsed,
  //     }));
  //   return departure;
  // }, [filteredDeparture, getTimeDelay]);

  // const jumlahDelay = departureDelay.length;
  ...
  // if (Array.isArray(filteredDepartures) && filteredDepartures.length > 0) {
  //   console.log(filteredDepartures, "ini ditampilkan"); // aman digunakan
  // }
  ...
  const navigateTime = (hours) => {
    ...
    // Buat tanggal baru dengan jam yang sudah dibatasi
    const newDate = new Date(base);

    // Jika jam keluar dari range 0-23, batasi saja tanpa ganti tanggal
    if (newHour < 0) {
    ...
    // const baseDateString = base.toDateString();
    ...
    //   let newHour = prevDate.getHours() + hours;
    ...
    // const boundedHour = Math.min(Math.max(newHour, 0), 23);

    // const newDate = new Date(prevDate);
    // newDate.setHours(boundedHour, 0, 0, 0);

    // const newDate = new Date(year, month, day, newHour, 0, 0, 0);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      }, 100); // jeda pendek untuk memastikan state sudah update
    }, 60 * 1000);
